...
export const authService = {
  ...
  async updateUser(attributes: UpdateAttributes): Promise<{ user: AuthUser | null; error: AuthError | null }> {
    try {
...
export function useAuth() {
  ...
  async function fetchProfile(userId: string) {
    try {
      ...
      if (pro) {
        setProfile({ ...pro, role: "professional", other_certifications: [] } as unknown as UserMetadata);
      } else {
        // Fetch generic profile and check role
        const { data: profileData } = await supabase
          .from("profiles")
          .select("id, email, full_name, phone, avatar_url, bio, address, postal_code, city, latitude, longitude, role, created_at, updated_at")
          .eq("id", userId)
          .single();
        
        if (profileData) {
          setProfile({ ...profileData, role: profileData.role || "particulier" } as UserMetadata);
        }
      }
    } catch (error) {

...
export const notificationService = {
  ...
  async notifyNewProject(project: Project) {
    try {
      ...
      const { data: admins, error: adminsError } = await supabase
        ...
        .in('email', ['admin@swipetonpro.fr', 'support@swipetonpro.fr']); // Admin pour validation, support pour aide
      ...
      console.log(
        `✅ Notifications nouveau projet envoyées à ${notifications.length} admins (admin pour validation, support pour aide)`
      );
      return { success: true };
    } catch (error) {
   ...
   */
  async notifyNewProfessionalRequest(
    projectId: string,
    ...
    try {
      ...
      console.log(
        `✅ Notification demande professionnelle envoyée (${requestCount + 1}/3)`
      );
      return { success: true };
    } catch (error) {
   ...
   */
  async notifyProfessionalPaused(
    projectId: string,
    clientEmail: string,
    ...
    try {
      ...
      await this.sendEmailNotification(
        professionalEmail,
        'professional-paused-professional',
        {
          professionalName,
          clientName,
   ...
   */
  async notifyProfessionalRejected(
    projectId: string,
    clientEmail: string,
    professionalEmail: string,
    professionalName: string,
    ...
    try {
      ...
      await this.sendEmailNotification(
        professionalEmail,
        'professional-rejected-professional',
        {
          professionalName,
          clientName,
   ...
   */
  async notifyProfessionalExpired(
    projectId: string,
    clientEmail: string,
    professionalEmail: string,
    professionalName: string,
    ...
    try {
      ...
      await this.sendEmailNotification(
        ...
        'matching-completed-client',
        {
          ...
          professionalCompany:
            professionalData?.company_name || 'Entreprise du professionnel', // Visible après paiement
          professionalEmail: professionalEmail, // Visible après paiement
          professionalPhone:
            professionalData?.phone ||
            'Coordonnées disponibles dans le dashboard', // Visible après paiement
          message:
      ...
      await this.sendEmailNotification(
        ...
        'matching-completed-professional',
        {
          ...
          clientEmail,
          clientLocation:
            clientData?.city || 'Localisation disponible dans le dashboard', // Visible après paiement
          message:
      ...
      const { data: internalAccounts, error: internalError } = await supabase
        .from('profiles')
        .select('id, email, full_name')
        .in('email', ['teamswipeTP@swipetonpro.fr', 'contact@swipetonpro.fr']);

      if (!internalError && internalAccounts) {
        for (const account of internalAccounts) {
          await this.sendEmailNotification(
            account.email,
            'matching-internal-notification',
            {
      ...
      console.log(
        '✅ Notifications matching complété envoyées (client + pro + interne)'
      );
      return { success: true };
    } catch (error) {
   ...
   */
  async notifyProjectValidated(
    project: Project,
    clientProfile: { full_name: string; email: string }
  ) {
    try {
      await this.sendEmailNotification(
        clientProfile.email,
        'project-validated-client',
        {
          clientName: clientProfile.full_name,
          projectTitle: project.title,

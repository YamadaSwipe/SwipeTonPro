...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    // Vérifier si nous avons une session de réinitialisation
    const checkResetSession = async () => {
      try {
        console.log('🔍 Checking reset session...');
        
        // Supabase gère automatiquement la session de réinitialisation
        // Il suffit de vérifier si nous avons une session active
        ...
        if (error) {
          ...
          toast({
            title: "❌ Erreur",
            description: "Impossible de vérifier la session de réinitialisation",
            variant: "destructive",
          });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!tokenValid) {
      setErrorMessage("Session invalide. Veuillez demander une nouvelle réinitialisation.");
      return;
    }
    ...
    if (password.toLowerCase() === 'password' || password.toLowerCase() === '12345678' || password.length < 8) {
      setErrorMessage("Choisissez un mot de passe plus sécurisé (8+ caractères, pas 'password' ou '12345678')");
      return;
    }
          ...
          <CardContent>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-600">
                <a href="/auth/login" className="text-orange-600 hover:text-orange-800 font-medium">

...
import { ArrowLeft, Upload, MessageSquare, CheckCircle, AlertCircle, Loader2, Sparkles, Award, Calendar, Euro } from "lucide-react";
import Link from "next/link";
import { useState, useEffect } from "react";
import { authService } from "@/services/authService";
import { projectService } from "@/services/projectService";
import { generateEstimationWithFallback, type AIEstimation } from "@/services/aiEstimationService";
...
type DiagnosticStep = "contact" | "project" | "photos" | "estimation" | "validation";
...
const CERTIFICATIONS = [
  { id: "qualibat", label: "Qualibat", description: "Certification qualité BTP" },
  { id: "rge", label: "RGE", description: "Reconnu Garant Environnement" },
  { id: "ecoArtisan", label: "Eco Artisan", description: "Rénovation énergétique" },
  { id: "qualitEnr", label: "Qualit'EnR", description: "Énergies renouvelables" },
  { id: "qualiPV", label: "QualiPV", description: "Panneaux solaires photovoltaïques" },
...
export default function DiagnosticPage() {
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setProjectData({ ...projectData, photos: [...projectData.photos, ...files] });
    }
  };
  ...
  const handleNext = async () => {
    if (currentStep === "contact") {
      if (projectData.firstName && projectData.lastName && projectData.email && projectData.phone) {
        // Vérifier si l'utilisateur est connecté à la fin de l'étape 1
        const session = await authService.getCurrentSession();
        if (!session?.user) {
          // Pas connecté, rediriger vers la création de compte
          console.log("⚠️ Utilisateur non connecté, redirection vers création de compte");
      ...
      if (projectData.description && projectData.workType && projectData.budgetMin && projectData.budgetMax && projectData.deadline) {
        // Vérifier le mode IA
  ...
  const handleValidateBudget = () => {
    if (projectData.budgetMax >= (projectData.aiEstimation?.estimation_min || 0)) {
  ...
  const handlePublishProject = async () => {
    ...
    try {
      ...
      if (projectData.photos.length > 0) {
        try {
          const { data: uploadedUrls, error: uploadError } = await projectService.uploadProjectImages(
            newProject.id, 
            projectData.photos
          );
      ...
      if (notificationSession?.user) {
        ...
        if (profile) {
          await notificationService.sendProjectRecap(newProject, profile as any);
        }
      }

      // Afficher un message de succès
      alert("✅ Projet publié avec succès!\n\nVotre projet est maintenant en attente de validation par nos modérateurs.\nVous recevrez un email de confirmation sur votre adresse: " + (notificationSession?.user?.email || projectData.email));
        ...
        <header className="border-b border-border bg-surface/80 backdrop-blur-md sticky top-0 z-50">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              <Link href="/particulier" className="flex items-center gap-2 text-text-secondary hover:text-primary transition-colors">
              ...
              <CardContent className="p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-text-secondary">Pour que les professionnels puissent vous contacter</p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      Vos coordonnées restent masquées jusqu'à validation mutuelle (système Double-Aveugle).
                    </AlertDescription>
                  </Alert>
                  ...
                  <Button
                    size="lg"
                    onClick={handleNext}
                    disabled={!projectData.firstName || !projectData.lastName || !projectData.email || !projectData.phone || !projectData.address}
              ...
              <CardContent className="p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-text-secondary">Décrivez précisément vos besoins</p>
                  </div>
                </div>

                <div className="space-y-6">
                  <div>
                    <Label htmlFor="workType" className="text-sm font-semibold mb-2">Type de travaux *</Label>
                    <select
                      id="workType"
                      value={projectData.workType}
                      onChange={(e) => setProjectData({ ...projectData, workType: e.target.value })}
                  ...
                  <div>
                    <Label htmlFor="location" className="text-sm font-semibold mb-2">Localisation (Ville) *</Label>
                    <Input
                      id="location"
                  ...
                  <div>
                    <Label htmlFor="surface" className="text-sm font-semibold mb-2">Surface (m²) - Optionnel</Label>
                    <Input
                      id="surface"
                      type="number"
                      placeholder="ex: 70"
                      value={projectData.surface || ""}
                      onChange={(e) => setProjectData({ ...projectData, surface: Number(e.target.value) || undefined })}
                      className="border-2"
                    />
                  </div>

                  <div>
                    <Label htmlFor="description" className="text-sm font-semibold mb-2">Description détaillée *</Label>
                    <Textarea
                      id="description"
                    ...
                    <p className="text-sm text-text-muted mt-2">
                      💡 <strong>Plus votre description est précise, meilleure sera l'estimation.</strong><br/>
                      N'hésitez pas à mentionner les marques souhaitées (ex: Bosch, Atlantic, Vaillant) - cette information aide l'IA et permet aux professionnels de mieux se projeter.
                    </p>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-primary/5 to-accent/5 rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="budgetMin" className="text-sm font-semibold mb-2">Budget minimum (€) *</Label>
                        <Input
                          id="budgetMin"
                          type="number"
                          placeholder="5000"
                          value={projectData.budgetMin || ""}
                          onChange={(e) => setProjectData({ ...projectData, budgetMin: Number(e.target.value) })}
                          className="border-2"
                        />
                      </div>
                      <div>
                        <Label htmlFor="budgetMax" className="text-sm font-semibold mb-2">Budget maximum (€) *</Label>
                        <Input
                          id="budgetMax"
                          type="number"
                          placeholder="15000"
                          value={projectData.budgetMax || ""}
                          onChange={(e) => setProjectData({ ...projectData, budgetMax: Number(e.target.value) })}
                          className="border-2"
                        />
                      </div>
                    </div>
                    {projectData.budgetMax > 0 && projectData.budgetMin > projectData.budgetMax && (
                      <p className="text-sm text-warning mt-2">⚠️ Le budget maximum doit être supérieur au minimum</p>
                    )}
                  </div>

                  {/* Deadline */}
                  <div>
                    <Label htmlFor="deadline" className="text-sm font-semibold mb-2 flex items-center gap-2">
                  ...
                  <div className="bg-gradient-to-br from-success/5 to-success/10 rounded-xl p-6">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-semibold">Certifications requises (optionnel)</h3>
                    </div>
                    <p className="text-sm text-text-secondary mb-4">
                      Sélectionnez les certifications que le professionnel doit posséder. 
                      <br/>Les pros sans ces certifications ne verront pas votre projet.
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {CERTIFICATIONS.map(cert => (
                        <div key={cert.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-white/50 transition-colors">
                          <Checkbox
                            id={cert.id}
                            checked={projectData.requiredCertifications.includes(cert.id)}
                            onCheckedChange={() => toggleCertification(cert.id)}
                          />
                          <div className="flex-1">
                            <Label htmlFor={cert.id} className="font-semibold cursor-pointer">{cert.label}</Label>
                    ...
                    {projectData.requiredCertifications.length > 0 && (
                      <Alert className="mt-4">
                        ...
                        <AlertDescription>
                          ✅ {projectData.requiredCertifications.length} certification(s) sélectionnée(s). 
                          Seuls les pros certifiés verront votre projet.
                        </AlertDescription>
                      </Alert>
                    )}
                  </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {aiSettings?.mode === "text_only" || !aiSettings?.enabled ? "Générer l'estimation" : "Ajouter des photos (optionnel)"}
                    </Button>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-2 animate-fade-in">
              <CardContent className="p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold">Photos de la zone de travaux</h2>
                    <p className="text-text-secondary">Optionnel - Aide l'IA à affiner son estimation</p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="border-2 border-dashed border-border rounded-xl p-8 text-center hover:border-primary transition-colors cursor-pointer">
                    ...
                    <label htmlFor="photo-upload" className="cursor-pointer">
                      ...
                      <p className="font-semibold mb-2">Cliquez pour ajouter des photos</p>
                  ...
                  {projectData.photos.length > 0 && (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      {projectData.photos.map((file, index) => (
                        <div key={index} className="relative aspect-square rounded-lg overflow-hidden border-2 border-success group">
                          <Image
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      <p className="text-sm mt-1">
                        Prenez des photos de l'ensemble de la pièce/zone, incluant les détails importants (murs, sol, plafond).
                      </p>
                    </AlertDescription>
                  </Alert>
                  ...
                  {aiSettings && !aiSettings.enabled && (
                    <Alert variant="destructive">
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription>
                        ⚠️ L'estimation IA est actuellement désactivée. L'estimation sera générée avec nos barèmes professionnels.
                      </AlertDescription>
                    </Alert>
                  )}
                  ...
                  {isProcessing ? (
                    <>
                      ...
                      <p className="text-text-secondary">
                        {aiSettings?.enabled ? "GPT-4 analyse votre projet" : "Calcul avec nos barèmes professionnels"}
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <h2 className="text-2xl font-bold mb-2">Estimation générée</h2>
                      <p className="text-text-secondary">
                        {projectData.estimationError ? "Estimation basée sur nos barèmes" : "Analyse complète"}
                      </p>
                    </>
                  )}
                </div>
                ...
                {!isProcessing && projectData.aiEstimation && (
                  <div className="space-y-6">
                    {projectData.estimationError && (
                      <Alert>
                        <AlertCircle className="h-4 w-4" />
                        <AlertDescription>{projectData.estimationError}</AlertDescription>
                      </Alert>
                    )}

                    {/* Estimation Range */}
                    <div className="bg-gradient-to-br from-primary/5 to-primary/10 rounded-2xl p-8 text-center">
                      <p className="text-sm font-semibold text-text-secondary mb-2">Fourchette recommandée</p>
                      <div className="flex items-center justify-center gap-4 mb-4">
                        <div>
                          <p className="text-3xl font-bold text-primary">
                            {projectData.aiEstimation.estimation_min.toLocaleString()}€
                          </p>
                          <p className="text-xs text-text-muted">Estimation basse</p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-text-muted">Estimation haute</p>
                        </div>
                      </div>
                    ...
                    {projectData.budgetMax > 0 && (
                      <Alert variant={
                        projectData.budgetMax < projectData.aiEstimation.estimation_min ? "destructive" :
                        projectData.budgetMin > projectData.aiEstimation.estimation_max ? "destructive" :
                        "default"
                      }>
                        <AlertCircle className="h-4 w-4" />
                        <AlertDescription>
                          {projectData.budgetMax < projectData.aiEstimation.estimation_min ? (
                            <p>⚠️ <strong>Budget insuffisant</strong> - Votre budget maximum ({projectData.budgetMax.toLocaleString()}€) est inférieur à l'estimation basse. Vous risquez de ne pas recevoir de propositions.</p>
                          ) : projectData.budgetMin > projectData.aiEstimation.estimation_max ? (
                            <p>💰 <strong>Budget généreux</strong> - Votre budget minimum ({projectData.budgetMin.toLocaleString()}€) dépasse l'estimation haute. Vous devriez recevoir beaucoup de propositions de qualité.</p>
                          ) : (
                            <p>✅ <strong>Budget cohérent</strong> - Votre budget ({projectData.budgetMin.toLocaleString()}€ - {projectData.budgetMax.toLocaleString()}€) correspond bien à l'estimation.</p>
                          )}
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    {projectData.aiEstimation.categories.length > 0 && (
                      <div className="space-y-3">
                        ...
                        {projectData.aiEstimation.categories.map((cat, index) => (
                    ...
                    {projectData.aiEstimation.risques.length > 0 && (
                      <div className="bg-warning/10 border border-warning/30 rounded-lg p-4">
                        ...
                        <ul className="space-y-1 text-sm">
                          {projectData.aiEstimation.risques.map((risk, index) => (
                    ...
                    {projectData.aiEstimation.conseils.length > 0 && (
                      <div className="bg-success/10 border border-success/30 rounded-lg p-4">
                        ...
                        <ul className="space-y-1 text-sm">
                          {projectData.aiEstimation.conseils.map((conseil, index) => (
                            <li key={index} className="flex items-start gap-2">
            ...
            <Card className="border-2 border-success animate-fade-in">
              <CardContent className="p-8">
                <div className="text-center mb-8">
                  ...
                  <p className="text-text-secondary">
                    Votre projet va être publié sur le flux des professionnels certifiés
                  </p>
                </div>
                ...
                <div className="space-y-4 mb-8">
                  <div className="bg-surface-elevated rounded-lg p-4">
                    ...
                    <p className="font-semibold">{projectData.firstName} {projectData.lastName}</p>
                    <p className="text-sm text-text-secondary">{projectData.email} • {projectData.phone}</p>
                  </div>
                  
                  <div className="bg-surface-elevated rounded-lg p-4">
                    <p className="text-sm text-text-muted mb-1">Adresse du chantier</p>
                    <p className="font-semibold">{projectData.address}</p>
                  </div>

                  <div className="bg-surface-elevated rounded-lg p-4">
                    <p className="text-sm text-text-muted mb-1">Type de travaux</p>
                  ...
                  <div className="bg-surface-elevated rounded-lg p-4">
                    <p className="text-sm text-text-muted mb-1">Délai souhaité</p>
                    <p className="font-semibold">{projectData.deadline}</p>
                  </div>

                  {projectData.requiredCertifications.length > 0 && (
                    <div className="bg-surface-elevated rounded-lg p-4">
                      <p className="text-sm text-text-muted mb-1">Certifications requises</p>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {projectData.requiredCertifications.map(certId => {
                          const cert = CERTIFICATIONS.find(c => c.id === certId);
                          return (
                            <span key={certId} className="px-3 py-1 rounded-full bg-success/10 text-success font-semibold text-sm">
                              {cert?.label}
                            </span>
                          );
                        })}
                  ...
                  <div className="bg-surface-elevated rounded-lg p-4">
                    <p className="text-sm text-text-muted mb-1">Photos uploadées</p>
                    <p className="font-semibold">{projectData.photos.length} photo{projectData.photos.length > 1 ? "s" : ""}</p>
                  </div>
                  
                  {projectData.aiEstimation && (
                    <div className="bg-gradient-to-br from-primary/5 to-accent/5 rounded-lg p-4">
                      <p className="text-sm text-text-muted mb-1">Estimation {aiSettings?.enabled ? "IA" : "professionnelle"}</p>
                      <p className="font-semibold text-lg">
                        {projectData.aiEstimation.estimation_min.toLocaleString()}€ - {projectData.aiEstimation.estimation_max.toLocaleString()}€
                      </p>
                      <p className="text-sm text-text-secondary mt-1">
                        Complexité: {projectData.aiEstimation.complexite} • Durée: {projectData.aiEstimation.duree_jours}j • Confiance: {projectData.aiEstimation.confidence_score}%
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg p-6 mb-6">
                  <h4 className="font-semibold mb-3">🔒 Protection Double-Aveugle activée</h4>
                  <ul className="space-y-2 text-sm text-text-secondary">
                    <li className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-success flex-shrink-0 mt-0.5" />
                      <span>Vos coordonnées restent masquées jusqu'à validation mutuelle</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-success flex-shrink-0 mt-0.5" />
                      <span>Vous recevrez les portfolios anonymisés des pros intéressés</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-success flex-shrink-0 mt-0.5" />
                      <span>
                    ...
                    <li className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-success flex-shrink-0 mt-0.5" />
                      <span>Aucun contact direct possible sans votre accord</span>
                    </li>
                  </ul>
                </div>

                ...
                >
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      {/* Intérêts reçus */}
                      {project.has_accepted_interest ? (
                        <div className="flex items-center text-sm text-emerald-600 font-medium">
                          <Users className="h-4 w-4 mr-1 flex-shrink-0" />
                          Professionnel accepté
                        </div>
                      ) : project.bids_count > 0 ? (
                        <div className="flex items-center text-sm text-orange-600 font-medium">
                          <Users className="h-4 w-4 mr-1 flex-shrink-0" />
                          {project.bids_count} professionnel
                          {project.bids_count > 1 ? 's' : ''} intéressé
                          {project.bids_count > 1 ? 's' : ''}
                        </div>
                      ) : null}
                      ...
                      <div className="pt-3 flex gap-2">
                        <Link
                          href={`/particulier/projects/${project.id}`}
                          className="flex-1"
                        >
                        ...
                        {project.bids_count > 0 && (
                          <Link
                            href={`/particulier/projects/${project.id}/interests`}
                            className="flex-1"
                          >
                            <Button className="w-full bg-orange-500 hover:bg-orange-600 text-white">
                              {project.bids_count} intérêt
                              {project.bids_count > 1 ? 's' : ''}
                            </Button>
                          </Link>
                        )}

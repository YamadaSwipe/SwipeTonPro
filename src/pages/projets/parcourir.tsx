  ...
  useEffect(() => {
    ...
    if (filterDelay) {
      filtered = filtered.filter((p) => {
        ...
        try {
          const analysis =
            typeof p.ai_analysis === 'string'
              ? JSON.parse(p.ai_analysis)
              : p.ai_analysis;
          const duration = analysis?.duration_days
            ? parseInt(analysis.duration_days)
            : null;
          if (!duration) return true; // Si pas de durée, on ne filtre pas
          switch (filterDelay) {
            case '1':
              return duration <= 7; // 1 semaine
            case '2':
              return duration <= 14; // 2 semaines
            case '3':
              return duration <= 30; // 1 mois
            case '4':
              return duration <= 60; // 2 mois
            case '5':
              return duration <= 90; // 3 mois
            default:
              return true;
          }
        } catch (error) {
          ...
          <div className="bg-card/50 backdrop-blur-sm border border-border rounded-lg p-6 mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <Select
                  value={filterWorkTypes.join(',')}
                  onValueChange={(value) =>
                    setFilterWorkTypes(value ? value.split(',') : [])
                  }
                >
                  <SelectTrigger>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {filterWorkTypes.length > 0 && (
                <Badge variant="secondary" className="gap-1">
                  {filterWorkTypes.length} type
                  {filterWorkTypes.length > 1 ? 's' : ''} sélectionné
                  {filterWorkTypes.length > 1 ? 's' : ''}
                </Badge>
              )}
              {filterCity && <Badge variant="secondary">{filterCity}</Badge>}
              {filterBudgetMin && (
                <Badge variant="secondary">Budget ≥ {filterBudgetMin}€</Badge>
              )}
              {filterBudgetMax && (
                <Badge variant="secondary">Budget ≤ {filterBudgetMax}€</Badge>
              )}
              {filterDelay && (
              ...
              {filterDistance && (
                <Badge variant="secondary">≤ {filterDistance}km</Badge>
              )}
            </div>
          </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {project.desired_deadline && (
                      <div className="flex items-center gap-2 text-text-secondary">
                        ...
                        <span className="text-sm">
                          {project.desired_deadline.toString()}
                        </span>
                      </div>
                    )}
                    ...
                    {project.required_certifications &&
                      project.required_certifications.length > 0 && (
                        <div className="pt-2 border-t border-border/50">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {project.required_certifications.map(
                              (cert, idx) => (
                                <Badge
                                  key={idx}
                                  variant="outline"
                          ...
                          {typeof project.ai_analysis === 'object' ? (
                            ...
                            <>
                              <p className="text-blue-600 font-medium">
                                Estimation chiffrée disponible
                              </p>

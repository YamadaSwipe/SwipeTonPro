...
export default function ProMatchesPage() {
  ...
  useEffect(() => {
    if (router.query.payment === 'success') {
      toast({
        ...
        description: 'Vous avez accès aux coordonnées du client.',
      });
    }
  }, [router.query]);
  ...
  const getStatusBadge = (status: string) => {
    const map: Record<string, { label: string; color: string }> = {
      ...
      payment_pending: {
        label: '🎉 Match ! Payez pour accéder',
    ...
    const cfg = map[status] || map.pre_matched;
    return (
      ...
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Gérez vos candidatures et débloquez vos matchs
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 space-y-4">
          {loading ? (
            ...
            matches.map((match) => {
              const conv = Array.isArray(match.conversation)
              ...
              return (
                <Card
                  key={match.id}
                  className={`transition-shadow hover:shadow-md ${match.status === 'payment_pending' ? 'border-yellow-300 bg-yellow-50/30' : ''}`}
                >
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {match.project?.estimated_budget_min?.toLocaleString(
                              'fr-FR'
                            )}
                            € –{' '}
                            {match.project?.estimated_budget_max?.toLocaleString(
                              'fr-FR'
                            )}
                            €
                          </span>
                        ...
                        <div className="flex items-center gap-2 text-sm text-yellow-700 bg-yellow-100 p-2 rounded-lg">
                          ...
                          <span>
                            ...
                            {Math.max(
                              0,
                              Math.floor(
                                (new Date(match.payment_deadline).getTime() -
                                  Date.now()) /
                                  3600000
                              )
                            )}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {match.status === 'payment_pending' && (
                        ...
                        >
                          ...
                          {paying === match.id
                            ? 'Redirection...'
                            : `Payer ${MISE_EN_RELATION_AMOUNT}€ de mise en relation`}
                        </Button>
                      )}

                      {(match.status === 'paid' ||
                        match.status === 'client_interested') &&
                        conv?.id && (
                          <Link href={`/chat/${conv.id}`} className="flex-1">
                            <Button
                              className="w-full"
                              variant={
                                match.status === 'paid' ? 'default' : 'outline'
                              }
                            >

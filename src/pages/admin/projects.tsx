...
export default function AdminProjectsPage() {
  ...
  const loadProjects = useCallback(async (silent = false) => {
    ...
    try {
      ...
      console.log('📊 Résultat projets admin:', {
        data: data?.length || 0,
        error,
      });
  ...
  const handleInfoNeeded = () => {
    ...
    updateStatus(infoModal!.id, 'info_needed', {
      info_needed_message: infoMessage,
    });
    setInfoModal(null);
    setInfoMessage('');
  };
  ...
  const budget = (p: Project) => {
    ...
    if (p.budget_min && p.budget_max)
      return `${p.budget_min.toLocaleString('fr-FR')}€ — ${p.budget_max.toLocaleString('fr-FR')}€`;
          ...
          <div className="flex gap-1 bg-gray-900 rounded-xl p-1 border border-white/8 overflow-x-auto">
            {TAB_CONFIG.map(({ key, label, color }) => (
              ...
              >
                {label}
                <span
                  className={`text-xs tabular-nums px-1.5 py-0.5 rounded-full bg-white/5 ${
            ...
            <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={
                    !infoMessage.trim() || actionLoading === infoModal.id
                  }

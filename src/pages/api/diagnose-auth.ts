  ...
  try {
    ...
    if (profilesError) {
      ...
      results.users = {
        ...
        byRole: profiles?.reduce((acc: any, user) => {
    ...
    try {
      ...
      if (authError) {
        ...
        const adminAuthUser = authUsers.users.find(u => u.email === 'admin@swipetonpro.fr');
        const bgreenAuthUser = authUsers.users.find(u => u.email === 'bgreen.rs@gmail.com');
    ...
    try {
      const { data: bgreenSignIn, error: bgreenSignInError } = await supabaseAdmin.auth.signInWithPassword({
    ...
    results.health = {
      ...
      status: criticalIssues > 0 ? 'CRITICAL' : highIssues > 0 ? 'WARNING' : 'HEALTHY',

  ...
  professionals: [
    {
      ...
      city: "Paris",
      postal_code: "75001",
      work_types: ["Plomberie", "Chauffage", "Climatisation"],
    ...
    },
    {
      ...
      city: "Lyon",
      postal_code: "69001",
      work_types: ["Électricité", "Domotique", "Sécurité"],
    ...
    },
    {
      ...
      city: "Marseille",
      postal_code: "13001",
      work_types: ["Menuiserie", "Fenêtres", "Portes", "Volets"],
  ...
  projects: [
    {
      ...
      description: "Je souhaite rénover complètement ma salle de bain de 8m². Installation d'une douche italienne, remplacement des carrelages, installation d'un nouveau meuble vasque et miroir.",
      category: "salle_de_bain",
      work_types: ["Plomberie", "Carrelage", "Électricité"],
    ...
    },
    {
      ...
      description: "Installation d'une climatisation réversible dans le salon de 35m². Travaux de passage des gaines et installation de l'unité extérieure.",
      category: "climatisation",
      work_types: ["Climatisation", "Électricité"],
    ...
    },
    {
      ...
      description: "Remplacement de 5 fenêtres en PVC double vitrage et installation de 3 volets roulants électriques.",
      category: "menuiserie",
      work_types: ["Menuiserie", "Fenêtres", "Électricité"],
  ...
  for (const pro of TEST_DATA.professionals) {
    try {
      ...
      if (authError) {
        ...
        results.push({ email: pro.email, success: false, error: authError.message });
        continue;
      }
      ...
      if (profileError) {
        console.error(`❌ Erreur création profil pour ${pro.email}:`, profileError);
        results.push({ email: pro.email, success: false, error: profileError.message });
        continue;
      }
...
async function createClients(supabase: any, res: NextApiResponse) {
  ...
  for (const client of TEST_DATA.clients) {
    try {
      ...
      if (authError) {
        console.error(`❌ Erreur création auth pour ${client.email}:`, authError);
        results.push({ email: client.email, success: false, error: authError.message });
        continue;
      }
      ...
      console.log(`✅ Compte client créé: ${client.first_name} ${client.last_name} (${client.email})`);
      results.push({ email: client.email, success: true, userId: authData.user.id });

    } catch (error: any) {
      console.error(`❌ Erreur création compte client ${client.email}:`, error);
      results.push({ email: client.email, success: false, error: error.message });
    }
  }
...
async function createProjects(supabase: any, res: NextApiResponse) {
  ...
  if (!clients || clients.length === 0) {
    return res.status(400).json({ error: "Aucun client trouvé pour créer des projets" });
  }
  ...
  for (let i = 0; i < TEST_DATA.projects.length && i < clients.length; i++) {
    ...
    try {
      ...
      if (projectError) {
        console.error(`❌ Erreur création projet ${project.title}:`, projectError);
        results.push({ title: project.title, success: false, error: projectError.message });
        continue;
      }
      ...
      results.push({ title: project.title, success: false, error: error.message });
    }
  }
...
async function simulateMatching(supabase: any, res: NextApiResponse) {
  ...
  if (!projects || !professionals) {
    return res.status(400).json({ error: "Aucun projet ou professionnel trouvé" });
  }
      ...
      try {
        const { data: interestData, error: interestError } = await supabase
          ...
          .insert({
            ...
            message: "Bonjour, je suis intéressé par votre projet et disponible pour le réaliser.",
            status: "pending",
            created_at: new Date().toISOString()
          })
          .select()
        ...
        if (interestError) {
          console.error(`❌ Erreur création intérêt: ${pro.company_name} -> ${project.title}`);
          continue;
        }
        ...
        console.error(`❌ Erreur postulation: ${pro.company_name} -> ${project.title}`, error);
      }
    }
  }
...
async function simulateDialogue(supabase: any, res: NextApiResponse) {
  ...
  for (const interest of interests) {
    try {
      ...
      const messages = [
        {
          ...
          sender: "client",
          content: "Bonjour, je suis intéressé par votre profil. Quand pouvez-vous commencer ?",
          created_at: new Date().toISOString()
        },
        {
          project_id: interest.project_id,
          professional_id: interest.professional_id,
          sender: "professional",
          content: "Bonjour ! Je suis disponible dès la semaine prochaine. Je peux vous faire un devis.",
      ...
      results.push({ interestId: interest.id, success: false, error: error.message });
    }
  }
...
async function simulatePayment(supabase: any, res: NextApiResponse) {
  ...
  if (!interests || interests.length === 0) {
    return res.status(400).json({ error: "Aucun intérêt en discussion trouvé" });
  }
  ...
  for (const interest of interests) {
    try {
      ...
      if (updateError) {
        console.error(`❌ Erreur acceptation intérêt ${interest.id}:`, updateError);
        continue;
      }
      ...
      if (paymentError) {
        console.error(`❌ Erreur paiement intérêt ${interest.id}:`, paymentError);
        continue;
      }
      ...
      console.log(`💳 Paiement simulé pour intérêt ${interest.id} (${paymentAmount}€)`);
      results.push({ 
        interestId: interest.id, 
        success: true, 
        amount: paymentAmount,
...
async function validateAdmin(supabase: any, res: NextApiResponse) {
  ...
  try {
    ...
    if (!pendingProjects || pendingProjects.length === 0) {
      return res.status(200).json({ message: "Aucun projet en attente de validation" });
    }
    ...
    for (const project of pendingProjects) {
      ...
      if (updateError) {
        console.error(`❌ Erreur validation projet ${project.id}:`, updateError);
        continue;
      }

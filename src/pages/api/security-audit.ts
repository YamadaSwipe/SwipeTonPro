...
export default async function handler(
  ...
  try {
    ...
    const emailValidationIssues =
      recentUsers?.filter(
        (user) =>
          !user.email || !user.email.includes('@') || user.email.length < 5
      ) || [];

    const phoneValidationIssues =
      recentUsers?.filter(
        (user) =>
          user.phone &&
          (user.phone.length < 10 || !user.phone.match(/^[+]?[\d\s-()]+$/))
      ) || [];
    ...
    auditResults.security.payments = {
      ...
      failedPayments:
        recentPayments?.filter((p) => p.status === 'failed')?.length || 0,
    ...
    const securityScores = Object.values(auditResults.security).map(
      (section: any) =>
    ...
    const overallRiskLevel =
      totalScore >= securityScores.length * 3
        ? 'CRITICAL'
        : totalScore >= securityScores.length * 2
          ? 'HIGH'
          : totalScore >= securityScores.length * 1.5
            ? 'MEDIUM'
            : 'LOW';
    ...
    auditResults.overall = {
      ...
      criticalIssues: Object.values(auditResults.security).filter(
        (section: any) => section.riskLevel === 'CRITICAL'
      ).length,
...
function generateRecommendations(security: any): string[] {
  ...
  if (security.authentication.hardcodedPasswords?.foundInLogin) {
    recommendations.push(
      "🚨 CRITIQUE: Remplacer le mot de passe admin en dur par une variable d'environnement"
    );
    recommendations.push(
      '🔐 Implémenter un système de hashage pour les mots de passe admin'
    );
  }
  ...
  if (security.payments.stripeConfig?.riskLevel === 'CRITICAL') {
    recommendations.push(
      '💳 CRITIQUE: Configurer les clés Stripe pour les paiements'
    );
  }
  ...
  if (security.apiEndpoints.riskLevel === 'MEDIUM') {
    ...
    recommendations.push(
      '🛡️ Ajouter des en-têtes de sécurité (CORS, CSP, etc.)'
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      '✅ Aucune recommandation critique - sécurité globale bonne'
    );
  }

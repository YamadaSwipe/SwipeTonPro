  ...
  try {
    ...
    if (listError) {
      return res.status(500).json({
        error: 'Erreur liste utilisateurs',
    ...
    if (authUser && !authUser.email_confirmed_at) {
      result.recommendations.push(
        "📧 L'email n'est pas confirmé. Regardez dans les spams."
      );
    }

    if (!profile && !profileError) {
      result.recommendations.push(
        "👤 L'utilisateur existe dans auth mais pas dans profiles. Contactez l'admin."
      );
    }

...
async function calculateKPIs(
  ...
  if (timeRange.start)
    projectsQuery = projectsQuery.gte('created_at', timeRange.start);
  if (timeRange.end)
    projectsQuery = projectsQuery.lte('created_at', timeRange.end);

  const { data: projects } = await projectsQuery;
  const totalProjects = projects?.length || 0;

  const projectsWithAIEstimation =
    projects?.filter((p: any) => {
      if (!p.ai_analysis && !p.ai_estimation) return false;
      const budgetDiff =
        p.estimated_budget_max && p.estimated_budget_min
          ? (p.estimated_budget_max - p.estimated_budget_min) /
  ...
  const projectKPIs = {
    ...
    byStatus: {
      pending_validation:
        projects?.filter((p: any) => p.status === 'pending_validation')
          .length || 0,
      validated:
        projects?.filter((p: any) => p.status === 'validated').length || 0,
      in_progress:
  ...
  const proIdsWithBids = new Set(
    bids?.map((b: any) => b.professional_id) || []
  );
  ...
  let clientProjectsQuery = supabase
    .from('projects')
    .select('client_id, status, assigned_to, accepted_bid_id');
  if (timeRange.start)
    clientProjectsQuery = clientProjectsQuery.gte(
      'created_at',
      timeRange.start
    );
  if (timeRange.end)
    clientProjectsQuery = clientProjectsQuery.lte('created_at', timeRange.end);
  ...
  const clientsWithProjects = new Set(
    clientsWithProjectsList.map((p: any) => p.client_id)
  ).size;

  const clientsWhoAccepted = clientsWithProjectsList.filter(
    (p: any) =>
  ...
  const matchesWithContract =
    matches?.filter((m: any) =>
      ['contract_signed', 'project_started', 'completed'].includes(m.status)
    ).length || 0;

  const rejectedMatches =
    matches?.filter((m: any) => ['rejected', 'cancelled'].includes(m.status))
      .length || 0;
  ...
  const matchKPIs = {
    ...
    rejectedMatches,
    rejectionRate:
      totalMatches > 0 ? Math.round((rejectedMatches / totalMatches) * 100) : 0,
    funnel: {
      match: totalMatches,

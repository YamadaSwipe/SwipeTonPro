...
export const TEST_DATA = {
  ...
  preMatchMessages: [
    {
      sender: "client",
      content: "Bonjour, je suis intéressé par votre profil pour mon projet. Avez-vous disponible ?"
    },
    {
      sender: "pro", 
      content: "Bonjour ! Oui je suis disponible. Pourriez-vous me donner plus de détails sur le projet ?"
    },
    {
      sender: "client",
    ...
    {
      sender: "pro",
      content: "Parfait. Je peux commencer dans 2 semaines. Je vous envoie un devis détaillé."
    }
  ],
...
export const TEST_SCRIPTS = {
  ...
  createProfessionals: async () => {
    ...
    for (const pro of TEST_DATA.professionals) {
      try {
        ...
        console.error(`❌ Erreur création compte pro: ${pro.company_name}`, error);
      }
    }
  },
  ...
  createClients: async () => {
    ...
    for (const client of TEST_DATA.clients) {
      try {
        ...
        if (response.ok) {
          console.log(`✅ Compte client créé: ${client.first_name} ${client.last_name}`);
        } else {
          console.error(`❌ Erreur création compte client: ${client.first_name} ${client.last_name}`);
        }
      } catch (error) {
        console.error(`❌ Erreur création compte client: ${client.first_name} ${client.last_name}`, error);
      }
    }
  },
  ...
  simulateMatching: async () => {
    ...
    const projects = await fetch("/api/projects/available").then(r => r.json());
    const professionals = await fetch("/api/professionals/list").then(r => r.json());
      ...
      for (const pro of professionals.data) {
        if (pro.work_types.some(wt => project.work_types.includes(wt))) {
          try {
            ...
            console.error(`❌ Erreur postulation: ${pro.company_name} -> ${project.title}`);
          }
        }
      }
    }
  ...
  simulatePreMatchDialogue: async () => {
    ...
    const interests = await fetch("/api/projects/interests").then(r => r.json());
  ...
  simulatePayment: async () => {
    ...
    const acceptedInterests = await fetch("/api/projects/interests?status=accepted").then(r => r.json());
  ...
  simulatePlanning: async () => {
    ...
    const matchedProjects = await fetch("/api/projects/matched").then(r => r.json());
    ...
    for (const project of matchedProjects.data) {
      for (const event of TEST_DATA.planningEvents) {
        try {
          ...
          console.log(`📅 Événement planning créé: ${event.type} le ${event.date}`);
        } catch (error) {
    ...
    const notifications = [
      {
        ...
        message: "Un nouveau projet correspondant à votre profil est disponible"
      },
      {

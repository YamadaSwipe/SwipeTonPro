...
export async function secureNotificationMiddleware(req: NextRequest) {
  try {
    ...
    const sensitiveTypes = ['professional_interested', 'project_accepted', 'match_completed'];
    
    if (sensitiveTypes.includes(req.body?.type) && !user) {
      return NextResponse.json(
    ...
    if (token && !checkRateLimit(token.sub as string)) {
      return NextResponse.json(
 ...
 */
export function withSecureNotification(handler: (req: NextRequest) => Promise<NextResponse>) {

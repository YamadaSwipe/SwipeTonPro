...
export function getSecurityStats() {
  ...
  const eventsLast24h = securityEvents.filter(
    (event) => new Date(event.timestamp) > last24h
  );
  const eventsLast1h = securityEvents.filter(
    (event) => new Date(event.timestamp) > last1h
  );
  ...
  const typeDistribution = eventsLast24h.reduce(
    (acc, event) => {
  ...
  const topIPs = eventsLast24h.reduce(
    (acc, event) => {
      if (event.ip !== 'unknown') {
  ...
  return {
    ...
    criticalEvents: securityEvents.filter((e) => e.riskLevel === 'CRITICAL')
      .length,
    lastCriticalEvent:
      securityEvents.filter((e) => e.riskLevel === 'CRITICAL')[0]?.timestamp ||
      null,
  };
}

// Nettoyer les anciens événements
export function cleanupOldEvents(maxAge: number = 7 * 24 * 60 * 60 * 1000) {
  // 7 jours par défaut

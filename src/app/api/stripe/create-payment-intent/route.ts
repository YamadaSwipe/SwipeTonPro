...
export async function POST(request: NextRequest) {
  try {
    ...
    const { amount, currency, description, metadata } = body as CreatePaymentIntentRequest;
    
    console.log('🔍 DEBUG: Request body received:', { amount, currency, description });
    
    if (!amount || !currency || !description) {
      console.error('❌ Stripe API: Missing required fields', { amount, currency, description });
    ...
    if (!AuthMiddleware.hasRole(authResult.role, 'professional')) {
      ...
      return AuthMiddleware.createPermissionError('Seul un professionnel peut acheter des crédits');
    }

    if (!authResult.professional) {
      console.error('❌ Stripe API: Professional profile not found');
      return AuthMiddleware.createPermissionError('Profil professionnel non trouvé');
    }

    console.log('✅ Stripe API: Professional validated:', authResult.professional.company_name);

    if (authResult.professional.status !== 'verified') {
      console.error('❌ Stripe API: Professional not verified', { status: authResult.professional.status });
      return AuthMiddleware.createPermissionError('Professionnel non vérifié');
    }

    console.log('✅ Stripe API: Professional verified:', authResult.professional.company_name);

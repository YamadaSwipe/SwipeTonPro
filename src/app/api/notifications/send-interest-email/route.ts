...
import { sendEmailWithRetry, generateProfessionalInterestEmail, generateAdminNotificationEmail } from '@/lib/mailer';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!projectId || !professionalId) {
      console.error('❌ Email API: Missing required fields', { projectId, professionalId });
      return NextResponse.json({ 
    ...
    console.log('✅ Email API: Professional validated:', professionalData.company_name);
    ...
    const { data: professionalDetails, error: professionalError } = await supabase
      .from('professionals')
      .select(`
        id,
        user_id,
    ...
    console.log('✅ Email API: Professional found:', professionalDetails.company_name);
    ...
    const clientEmailData = generateProfessionalInterestEmail({
      ...
      professionalName: professionalDetails.profiles.full_name,
...
export async function GET() {
  try {
    ...
    const isConfigured = Object.values(smtpConfig).every(status => status.includes('✅'));

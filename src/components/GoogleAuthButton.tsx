...
export default function GoogleAuthButton({ mode, userType, className }: GoogleAuthButtonProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?userType=${userType || 'client'}`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          }

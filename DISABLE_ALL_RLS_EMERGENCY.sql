-- SCRIPT D'URGENCE : DÉSACTIVER TOUT RLS POUR ARRÊTER LES ERREURS 406
-- Exécutez ce script si vous avez encore 1300+ erreurs en boucle
...
ALTER TABLE project_views DISABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Enable all access on project_views" ON project_views FOR ALL USING (true) WITH CHECK (true);
...
WHERE schemaname = 'public' 
    AND tablename IN ('profiles', 'professionals', 'projects', 'bids', 'messages', 'reviews', 'credit_transactions', 'notifications', 'documents', 'project_views')
ORDER BY tablename;
